...
class ContactLimitManager {
  ...
  async getCurrentLimits() {
    try {
      ...
      const validLimits = limits.filter(
        (record) => now - record.timestamp < this.TWENTY_FOUR_HOURS_MS
      );
      ...
      console.error(
        '[HP Extension Background] Error getting contact limits:',
        error
      );
      return [];
    }
  ...
  async recordImport(contactCount) {
    try {
      ...
      console.log(
        `[HP Extension Background] Recorded import of ${contactCount} contacts. Total in last 24h: ${await this.getTotalContactsInLast24Hours()}`
      );
    } catch (error) {
  ...
  async getTimeUntilReset() {
    ...
    const timeUntilReset =
      this.TWENTY_FOUR_HOURS_MS - (Date.now() - oldestRecord.timestamp);
...
class BackgroundService {
  constructor() {
    console.log(
      '[HP Extension Background] BackgroundService constructor called'
    );
  ...
  setupMessageHandlers() {
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      console.log(
        '[HP Extension Background] Received message:',
        message.type,
        message
      );
      switch (message.type) {
      ...
      case 'CAPTURE_COOKIES_FROM_WINDOW':
        console.log(
          '[HP Extension Background] Capturing cookies from window:',
          message.windowUrl
        );
      ...
      case 'CAPTURE_COOKIES_FROM_DOMAIN':
        console.log(
          '[HP Extension Background] Capturing cookies from domain:',
          message.domain
        );
      ...
      case 'FETCH_SEGMENTS':
        console.log(
          '[HP Extension Background] Fetching segments for workspace:',
          message.workspaceId
        );
      ...
      case 'CAPTURE_COOKIES_FROM_ACTIVE_TAB':
        console.log(
          '[HP Extension Background] Capturing cookies from active tab'
        );
        this.captureCookiesFromActiveTab(sendResponse);
        return true; // Keep message channel open for async response
      case 'CHECK_CONTACT_LIMIT':
        console.log(
          '[HP Extension Background] Checking contact limit for:',
      ...
      default:
        console.log(
          '[HP Extension Background] Unknown message type:',
          message.type
        );
      }
    });
  ...
  async checkContactLimit(contactCount, sendResponse) {
    try {
      ...
      const canImport =
        await this.contactLimitManager.canImportContacts(contactCount);
      const currentTotal =
        await this.contactLimitManager.getTotalContactsInLast24Hours();
      ...
      if (canImport) {
        sendResponse({
          ...
          remaining:
            this.contactLimitManager.MAX_CONTACTS_PER_24H - currentTotal
        });
      } else {
        const timeUntilReset =
          await this.contactLimitManager.getTimeUntilReset();
        const formattedTime =
          this.contactLimitManager.formatTimeUntilReset(timeUntilReset);
      ...
      console.error(
        '[HP Extension Background] Error checking contact limit:',
        error
      );
      sendResponse({
        success: false,
  ...
  async handleSalesNavImport(message, sender) {
    console.log(
      '[HP Extension Background] Starting sales nav import:',
      message
    );
    try {
      ...
      console.log(
        '[HP Extension Background] Connected to tab for progress updates'
      );
      ...
      console.log(
        '[HP Extension Background] Data fetching completed, records found:',
        data.length
      );
      ...
      console.log(
        '[HP Extension Background] Checking contact limit before import...'
      );
      const canImport = await this.contactLimitManager.canImportContacts(
        data.length
      );

      if (!canImport) {
        console.log(
          '[HP Extension Background] Contact limit exceeded, blocking import'
        );
        const timeUntilReset =
          await this.contactLimitManager.getTimeUntilReset();
        const formattedTime =
      ...
      console.log(
        '[HP Extension Background] Sending data to Highperformr API...'
      );
      const isCompanySearch =
        message.searchType && message.searchType.includes('Company');
      await this.sendToHighperformr(
        data,
        message.workspace,
        message.segment,
        port,
        isCompanySearch ? 'company' : 'people'
      );
  ...
  async getLinkedInCookies() {
    try {
      ...
      throw new Error(
        'Failed to access LinkedIn cookies. Please ensure you are logged into LinkedIn.'
      );
    }
  }
  ...
  async fetchSalesNavData(searchUrl, searchType, port) {
    ...
    console.log('search page from sales navi - ', searchUrl);
    port.postMessage({
    ...
    while (hasMore && page <= maxPages) {
      try {
        ...
        console.log(
          `[HP Extension Background] Generated API URL for page ${page}:`,
          apiUrl
        );

        const response = await this.fetchLinkedInAPI(apiUrl);
        console.log(
          `[HP Extension Background] LinkedIn API response for page ${page}:`,
          {
        ...
        console.log(
          `[HP Extension Background] Processed data for page ${page}:`,
          {
        ...
        console.log(
          `[HP Extension Background] Page ${page}: Got ${currentBatchSize} contacts, Total: ${allData.length}, HasMore: ${hasMore}, MaxContacts: ${maxContacts}`
        );

        if (currentBatchSize < 25) {
          console.log(
            `[HP Extension Background] Stopping because batch size (${currentBatchSize}) is less than 25`
          );
        }
        if (allData.length >= maxContacts) {
          console.log(
            `[HP Extension Background] Stopping because reached max contacts (${maxContacts})`
          );
        }
  ...
  async fetchLinkedInAPI(url) {
    ...
    const response = await fetch(url, {
      headers: {
        ...
        'x-li-identity':
          'dXJuOmxpOmVudGVycHJpc2VQcm9maWxlOih1cm46bGk6ZW50ZXJwcmlzZUFjY291bnQ6MzQzNjA1MjM0LDM1NDc2NTQ5NCk',
    ...
    if (!response.ok) {
      if (response.status === 401) {
        throw new Error(
          'LinkedIn authentication expired. Please refresh the page and try again.'
        );
      } else if (response.status === 403) {
        throw new Error(
          'Access denied. Please ensure you have Sales Navigator access.'
        );
  ...
  createSalesNaviApiUrl(salesUrl, page = 1) {
    try {
      ...
      const sharedSearchParam = params.get('sharedSearchId');
      ...
      } else if (sharedSearchParam) {
        const parts = sharedSearchParam.split(',');
        if (parts.length >= 3) {
          const sharerSeatId = parts[0];
          const sharedSearchId = parts[2];
          return `https://www.linkedin.com/sales-api/salesApiLeadSearch?q=sharedSearchId&start=${start}&count=25&sharedSearchId=${sharedSearchId}&sharerSeatId=${sharerSeatId}&trackingParam=(sessionId:${sessionId})&decorationId=${decoration}`;
        }
      }
  ...
  createCompanySalesNaviApiUrl(salesUrl, page = 1) {
    try {
      ...
      // Handle shared searches
      else if (sharedSearchParam) {
        const parts = sharedSearchParam.split(',');
        if (parts.length >= 3) {
          const sharerSeatId = parts[0];
          const sharedSearchId = parts[2];
          return `https://www.linkedin.com/sales-api/salesApiAccountSearch?q=sharedSearchId&start=${start}&count=25&sharedSearchId=${sharedSearchId}&sharerSeatId=${sharerSeatId}&trackingParam=(sessionId:${sessionId})&decorationId=${decoration}`;
        }
        return null;
      }
  ...
  processPeopleData(data) {
    try {
      ...
      const transformedData = data.elements
        .map((element) => {
          try {
            ...
            // Extract spotlight badge information
            let mutualConnection = '';
            let recentlyHiredOrPromoted = '';
            let recentlyPosted = '';

            if (
              element.spotlightBadges &&
              Array.isArray(element.spotlightBadges)
            ) {
              element.spotlightBadges.forEach((badge) => {
                const displayValue = badge.displayValue || '';
                const lowerValue = displayValue.toLowerCase();

                if (lowerValue.includes('mutual connection')) {
                  // Extract just the number from "X mutual connections"
                  const match = displayValue.match(/\d+/);
                  mutualConnection = match ? match[0] : displayValue;
                } else if (
                  lowerValue.includes('promoted') ||
                  lowerValue.includes('hired')
                ) {
                  recentlyHiredOrPromoted = displayValue;
                } else if (lowerValue.includes('posted')) {
                  recentlyPosted = displayValue;
                }
              });
            }
            ...
            return {
              ...
              RecentlyHiredOrPromoted: recentlyHiredOrPromoted,
      ...
      const transformedData = data.elements
        .map((element) => {
          try {
            ...
            const linkedinUrl = entityId
              ? `linkedin.com/company/${entityId}`
              : '';
  ...
  async sendToHighperformr(
    ...
    try {
      ...
      if (!accountId || accountId === 'unknown') {
        console.log(
          '[HP Extension Background] AccountId not found or unknown, using workspaceId as fallback:',
          workspaceId
        );
      ...
      console.log(
        '[HP Extension Background] Using accountId for API calls:',
        accountId
      );
      ...
      if (searchType === 'company') {
        ...
        fieldMapping = [
          ...
          {
            sourceFieldName: 'RecentlyHiredOrPromoted',
            hpFieldName: 'contact.recentlyHiredOrPromoted'
          },
          {
            sourceFieldName: 'RecentlyPosted',
            hpFieldName: 'contact.recentlyPosted'
          },
          {
            sourceFieldName: 'MutualConnection',
            hpFieldName: 'contact.mutualConnection'
          },
          {
            sourceFieldName: 'contactRecord',
            hpFieldName: 'contact.contactRecord'
      ...
      console.log(
        '  - URL:',
        `${api.baseURL}/api/sources/bulk-upsert?accountId=${accountId}`
      );
      ...
      console.log(
        '  - URL:',
        `${api.baseURL}/api/contacts/${sourceId}/bulk-upsert-contacts?accountId=${accountId}`
      );
      console.log('  - Request body:', { contactsData: data });

      const contactsResponse = await api.addContactsToSource(
        accountId,
        sourceId,
        data
      );
      ...
      console.log(
        '📞 AddContactsToSource response status:',
        contactsResponse.status
      );
      console.log(
        '✅ AddContactsToSource response data:',
      ...
      throw new Error(
        `Failed to send data to Highperformr.ai: ${error.message}`
      );
    }
  }
  ...
  async updateSegmentWithSource(accountId, sourceId, segment) {
    try {
      ...
      if (cookies) {
        ...
        if (
          cookies.includes('_hp_auth_session') &&
          !cookies.includes('session=')
        ) {
          console.log(
            '[HP Extension Background] Using _hp_auth_session as session cookie for updateSegmentWithSource'
          );
          ...
          if (authSessionMatch) {
            ...
            console.log(
              '[HP Extension Background] Updated updateSegmentWithSource cookie string with session cookie'
            );
          }
        }
      }

      const getSegmentResponse = await fetch(
        `https://app.highperformr.ai/api/segments/${segmentId}?accountId=${accountId}`,
        {
          method: 'GET',
          headers: headers,
          credentials: 'include'
      ...
      if (!getSegmentResponse.ok) {
        throw new Error(
          `Failed to get segment data: ${getSegmentResponse.status}`
        );
      }
      ...
      const existingConditionValues =
        existingSegment.data?.attributes?.condition?.value || [];
      const existingSegmentSources =
        existingSegment.data?.attributes?.segmentSources || [];
      ...
      const mergedConditionValues = [
        ...new Set([...existingConditionValues, sourceId])
      ];
      const mergedSegmentSources = [
        ...new Set([...existingSegmentSources, sourceId])
      ];
      ...
      if (verifyResponse.ok) {
        ...
        console.log(
          '✅ Segment state AFTER PATCH:',
          segmentAfterPatch.data?.attributes?.segmentSources
        );
        console.log(
          '✅ Condition AFTER PATCH:',
          segmentAfterPatch.data?.attributes?.condition
        );
      }

      console.log(
        '✅ Segment update completed - contacts now visible in segment'
      );
    } catch (error) {
  ...
  async getHighperformrCookies(sendResponse) {
    try {
      ...
      console.log(
        '[HP Extension Background] Cookie string length:',
        cookieString.length
      );
      ...
      console.error(
        '[HP Extension Background] Error getting Highperformr cookies:',
        error
      );
  ...
  async captureCookiesFromWindow(windowUrl, sendResponse) {
    try {
      ...
      console.log(
        '[HP Extension Background] Extracting cookies for domain:',
        domain
      );
      ...
      console.log(
        '[HP Extension Background] Found cookies for domain:',
        cookies.length
      );

      // Also get cookies for parent domains (like .highperformr.ai)
      const parentDomain = domain.startsWith('.')
        ? domain
        : `.${domain.split('.').slice(-2).join('.')}`;
      console.log(
        '[HP Extension Background] Also checking parent domain:',
        parentDomain
      );
      ...
      console.log(
        '[HP Extension Background] Found parent domain cookies:',
      ...
      const uniqueCookies = allCookies.reduce((acc, cookie) => {
        if (
          !acc.find((c) => c.name === cookie.name && c.domain === cookie.domain)
        ) {
          acc.push(cookie);
        }
        return acc;
      ...
      console.log(
        '[HP Extension Background] Unique cookies after deduplication:',
      ...
      console.log(
        '[HP Extension Background] Captured cookie string length:',
        cookieString.length
      );
      console.log(
        '[HP Extension Background] Captured cookies text:',
        cookieString
      );
      ...
      console.log(
        '[HP Extension Background] Cookies stored in extension storage'
      );
      ...
      console.error(
        '[HP Extension Background] Error capturing cookies from window:',
        error
      );
      sendResponse({
        success: false,
  ...
  async captureCookiesFromDomain(domain, sendResponse) {
    try {
      ...
      console.log(
        '[HP Extension Background] Trying to capture cookies from domains:',
        domainVariations
      );
      ...
      for (const domainVar of domainVariations) {
        try {
          ...
          console.log(
            `[HP Extension Background] Found ${cookies.length} cookies for domain: ${domainVar}`
          );
          ...
          cookies.forEach((cookie) => {
            ...
            const logPrefix = isSessionCookie
              ? '🔑 SESSION COOKIE:'
              : '[HP Extension Background] Cookie:';
            console.log(
              `${logPrefix} ${cookie.name} (domain: ${cookie.domain}, path: ${cookie.path}, secure: ${cookie.secure}, httpOnly: ${cookie.httpOnly}, value: ${cookie.value.substring(0, 50)}...)`
            );

            if (cookie.name === 'session') {
              console.log(
                '🎯 FOUND THE SESSION COOKIE! Domain:',
                cookie.domain,
                'Path:',
                cookie.path
              );
          ...
          console.log(
            `[HP Extension Background] Error getting cookies for ${domainVar}:`,
            error.message
          );
        }
      }
      ...
      console.log(
        '[HP Extension Background] Total unique cookies found:',
        uniqueCookies.length
      );
      ...
      importantCookies.forEach((cookieName) => {
        ...
        if (cookie) {
          console.log(
            `[HP Extension Background] Found important cookie ${cookieName}: ${cookie.value.substring(0, 50)}...`
          );
          console.log(
            `[HP Extension Background] Cookie ${cookieName} domain: ${cookie.domain}, path: ${cookie.path}, secure: ${cookie.secure}, httpOnly: ${cookie.httpOnly}`
          );
        } else {
          console.log(
            `[HP Extension Background] Missing important cookie: ${cookieName}`
          );
        }
      });
      ...
      if (!sessionCookie) {
        console.log(
          '[HP Extension Background] CRITICAL: No session cookie found! This will cause 401 errors.'
        );

        // Try to find any cookies that might be the session cookie
        const possibleSessionCookies = uniqueCookies.filter(
          (c) =>
        ...
        console.log(
          '[HP Extension Background] Possible session cookies found:',
          possibleSessionCookies.map((c) => c.name)
        );
      } else {
      ...
      console.error(
        '[HP Extension Background] Error capturing cookies from domain:',
        error
      );
      sendResponse({
        success: false,
  ...
  async captureCookiesFromActiveTab(sendResponse) {
    try {
      console.log(
        '[HP Extension Background] Attempting to capture cookies from active tab'
      );
      ...
      if (!activeTab.url || !activeTab.url.includes('highperformr.ai')) {
        console.log(
          '[HP Extension Background] Active tab is not on highperformr.ai domain'
        );
        sendResponse({
          success: false,
          error: 'Active tab not on highperformr.ai'
        });
        return;
      }
      ...
      const domain = url.hostname;
      console.log(
        '[HP Extension Background] Extracting cookies for active tab domain:',
        domain
      );

      const cookies = await chrome.cookies.getAll({ url: activeTab.url });
      console.log(
        '[HP Extension Background] Found cookies for active tab:',
        cookies.length
      );
      ...
      cookies.forEach((cookie) => {
        const isSessionCookie =
          cookie.name === 'session' || cookie.name.includes('session');
        const logPrefix = isSessionCookie
          ? '🔑 ACTIVE TAB SESSION COOKIE:'
          : '[HP Extension Background] Active tab cookie:';
        console.log(
          `${logPrefix} ${cookie.name} (domain: ${cookie.domain}, value: ${cookie.value.substring(0, 50)}...)`
        );
      });
      ...
      console.log(
        '[HP Extension Background] Active tab cookie string length:',
        cookieString.length
      );
      ...
      console.error(
        '[HP Extension Background] Error capturing cookies from active tab:',
        error
      );
    ...
    try {
      ...
      if (cookies) {
        ...
        if (
          ...
          console.log(
            '[HP Extension Background] Using _hp_auth_session as session cookie for workspaces'
          );
          const authSessionMatch = cookies.match(/_hp_auth_session=([^;]+)/);
          if (authSessionMatch) {
            const sessionValue = authSessionMatch[1];
            headers['Cookie'] = `${cookies}; session=${sessionValue}`;
            console.log(
              '[HP Extension Background] Updated workspaces cookie string with session cookie'
            );
          }
        }
      }
      ...
      const response = await fetch(
        'https://app.highperformr.ai/api/users/session',
        {
          method: 'GET',
          headers: headers,
          credentials: 'include'
      ...
      console.log(
        '[HP Extension Background] Workspaces response status:',
        response.status
      );
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        console.log(
          '[HP Extension Background] Fetched user data for workspaces:',
          userData
        );
        ...
        console.log(
          '[HP Extension Background] Extracted workspaces:',
          workspaces.length
        );
        ...
        console.log(
          '[HP Extension Background] Response is not JSON, returning empty workspaces'
        );
        sendResponse({
          success: true,
          workspaces: []
      ...
      console.error(
        '[HP Extension Background] Error fetching workspaces:',
        error
      );
      sendResponse({
        success: false,
    ...
    try {
      ...
      if (cookies) {
        ...
        if (
          ...
          console.log(
            '[HP Extension Background] Using _hp_auth_session as session cookie for segments'
          );
          const authSessionMatch = cookies.match(/_hp_auth_session=([^;]+)/);
          if (authSessionMatch) {
            const sessionValue = authSessionMatch[1];
            headers['Cookie'] = `${cookies}; session=${sessionValue}`;
            console.log(
              '[HP Extension Background] Updated segments cookie string with session cookie'
            );
          }
        }
      }
      ...
      const response = await fetch(
        `https://app.highperformr.ai/api/segments/filter-segments?accountId=${workspaceId}`,
        {
          method: 'POST',
          headers: headers,
      ...
      console.log(
        '[HP Extension Background] Segments response status:',
        response.status
      );
      ...
      console.log(
        '[HP Extension Background] Segments response data:',
        segmentsData
      );
      ...
      console.log(
        '[HP Extension Background] Extracted segments:',
        segments.length
      );
      ...
      console.log(
        '[HP Extension Background] Mapped segments:',
      ...
      console.error(
        '[HP Extension Background] Error fetching segments:',
        error
      );
      sendResponse({
        success: false,
  ...
  async verifySession(sendResponse) {
    try {
      ...
      const cookies = await this.getStoredCookies();
      console.log(
        '[HP Extension Background] Using cookies for verification (length):',
        cookies.length
      );
      ...
      if (cookies) {
        ...
        importantCookies.forEach((cookieName) => {
          if (cookies.includes(cookieName)) {
            console.log(
              `[HP Extension Background] Found ${cookieName} in cookie string`
            );
          } else {
            console.log(
              `[HP Extension Background] Missing ${cookieName} in cookie string`
            );
          }
        });
        ...
        console.log(
          '[HP Extension Background] Using captured cookies directly'
        );
        ...
        console.log(
          '[HP Extension Background] Has session cookie:',
          hasSessionCookie
        );
        console.log(
          '[HP Extension Background] Has _hp_auth_session cookie:',
          hasAuthSessionCookie
        );

        if (!hasSessionCookie) {
          console.log(
            '[HP Extension Background] WARNING: No session cookie found - this will likely cause 401 error'
          );
          console.log(
            '[HP Extension Background] Available cookies:',
            cookies
              .split(';')
          ...
          console.log(
            '[HP Extension Background] SUCCESS: Found session cookie in request'
          );
        }

        console.log(
          '[HP Extension Background] Final cookie string length:',
          headers['Cookie'].length
        );
      } else {
        console.log(
          '[HP Extension Background] No cookies available for session verification'
        );
      }

      console.log(
        '[HP Extension Background] Making session API call with headers:',
        headers
      );
      ...
      console.log(
        '[HP Extension Background] Session verification response status:',
        response.status
      );
      console.log(
        '[HP Extension Background] Session verification response headers:',
      ...
      if (response.ok) {
        const contentType = response.headers.get('content-type');
        console.log(
          '[HP Extension Background] Response content-type:',
          contentType
        );
        ...
        if (contentType && contentType.includes('application/json')) {
          const userData = await response.json();
          console.log(
            '[HP Extension Background] Session verification successful, user data:',
            userData
          );
          ...
          console.log(
            '[HP Extension Background] Got 200 but not JSON - considering session valid'
          );
          const responseText = await response.text();
          console.log(
            '[HP Extension Background] Response text (first 200 chars):',
        ...
        console.log(
          '[HP Extension Background] Session verification failed with status:',
          response.status
        );
        ...
        try {
          ...
          console.log(
            '[HP Extension Background] Error response body:',
            errorText
          );
        } catch (e) {
          console.log(
            '[HP Extension Background] Could not read error response body'
          );
        }
      ...
      console.error(
        '[HP Extension Background] Session verification error:',
        error
      );
      sendResponse({
        success: false,
  ...
  async getStoredCookies() {
    ...
    try {
      ...
      console.log(
        '[HP Extension Background] Stored cookies length:',
        cookies.length
      );
      return cookies;
    } catch (error) {
      console.error(
        '[HP Extension Background] Error getting stored cookies:',
        error
      );
      return '';
    }
...
class HighperformrAPI {
  ...
  async createSource(accountId, sourceData) {
    console.log(
      '[HP Extension Background] Creating source with accountId:',
      accountId
    );
    const cookies = await this.getStoredCookies();
    ...
    if (cookies) {
      ...
      if (
        ...
        console.log(
          '[HP Extension Background] Using _hp_auth_session as session cookie for createSource'
        );
        const authSessionMatch = cookies.match(/_hp_auth_session=([^;]+)/);
        if (authSessionMatch) {
          const sessionValue = authSessionMatch[1];
          headers['Cookie'] = `${cookies}; session=${sessionValue}`;
          console.log(
            '[HP Extension Background] Updated createSource cookie string with session cookie'
          );
        }
      }
    }
    ...
    const response = await fetch(
      `${this.baseURL}/api/sources/bulk-upsert?accountId=${accountId}`,
      {
        method: 'POST',
        headers: headers,
    ...
    console.log(
      '[HP Extension Background] Create source response status:',
      response.status
    );
  ...
  async addContactsToSource(accountId, sourceId, contactsData) {
    console.log(
      '[HP Extension Background] Adding contacts to source:',
      sourceId
    );
    const cookies = await this.getStoredCookies();
    const headers = { 'Content-Type': 'application/json' };
    ...
    if (cookies) {
      ...
      if (
        ...
        console.log(
          '[HP Extension Background] Using _hp_auth_session as session cookie for addContactsToSource'
        );
        const authSessionMatch = cookies.match(/_hp_auth_session=([^;]+)/);
        if (authSessionMatch) {
          const sessionValue = authSessionMatch[1];
          headers['Cookie'] = `${cookies}; session=${sessionValue}`;
          console.log(
            '[HP Extension Background] Updated addContactsToSource cookie string with session cookie'
          );
        }
      }
    }
    ...
    // Use the exact API endpoint from the network logs
    const response = await fetch(
      `${this.baseURL}/api/contacts/${sourceId}/bulk-upsert-contacts?accountId=${accountId}`,
      {
        method: 'POST',
        headers: headers,
    ...
    console.log(
      '[HP Extension Background] Add contacts response status:',
      response.status
    );
